...
class MonaiModelTest():
    def __init__(self, monai_model_file):
        self.monai_model = monai.networks.nets.densenet.densenet121(spatial_dims=3, in_channels=3, out_channels=out_dim).to("cuda")
        self.monai_model.load_state_dict(torch.load(monai_model_file), strict=True)
        self.monai_model.eval()  
    
    def pred_monai(self, img_monai):
        monai_preds = torch.sigmoid( self.monai_model(img_monai) ).cpu().detach().numpy().squeeze()
        return monai_preds
