...
def main():
    ...
    # b3 nonweight calib: [3.8250639579850194, 8.555037588568537, 4.374239635034443, 7.480972390526775, 5.002262078458348]
    img_models = {
    ...
    for _target in [ 'rv_lv_ratio_gte_1', "central_pe", "leftsided_pe", "rightsided_pe", "chronic_pe", "rv_lv_ratio_lt_1"]:
        lgb_models_each_target[_target] = [pickle.load(open(f'lgb_models/b3_exams_monai_1026/{_target}_monai_lgb_seed0_fold{i}.pkl', 'rb')) for i in range(5)]
    ...
    result_df_dict, monai_df = sub_5(None, img_models)
...
def sub_5(cfg, img_models):
    ...
    for (item) in tqdm(dataloader):
        ...
        monai_result_arr.append(dict(
            {"StudyInstanceUID": study_id},
            ** dict( zip( monai_targets,  pred_monai ) )
        ))
    ...
    return result_df_dict, monai_df
